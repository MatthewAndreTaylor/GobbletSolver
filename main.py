...
class GameGUI:
    def __init__(self, root):
        ...
        starting_board = [ [[-1] , [-1], [-1]],
                   [[-1] , [-1], [-1]],
                   [[-1] , [-1], [-1]] ]

        player1 = Player(frozenset([0, 1, 2, 3, 4, 5]), frozenset([0, 1, 2, 3, 4, 5])) # player 1 has pieces 0-5
        player2 = Player(frozenset([6, 7, 8, 9, 10, 11]), frozenset([6, 7, 8, 9, 10, 11])) # player 2 has pieces 6-11
    ...
    def update_selection(self):
        ...
        for pid in player.unused_pieces:
            ...
            btn = tk.Button(self.piece_frame,
                                text=f"{color}-{power}",
        ...
        for i in range(3):
            for j in range(3):
                ...
                if pid in self.game.players[self.current_player].pieces:
                    self.buttons[i][j].configure(command=partial(self.make_selection, (pid, (i, j))))
    ...
    def make_selection(self, selection):
        ...
        for i in range(3):
            for j in range(3):
                self.buttons[i][j].configure(command=lambda i=i, j=j: self.on_board_click(i, j))
        ...
        if self.selection:
            ...
            if move in get_possible_moves(self.game, self.current_player):
                ...
                if self.current_player == 1:
                    ...
                    best_move, score = select_move_alphabeta(self.game, self.current_player, 4)

                    print("Best Move: ", best_move, "color: ", get_color(best_move.pid), "power: ", get_power(best_move.pid))
                    print("Score: ", score)
    ...
    def update_board(self):
        ...
        if compute_utility(self.game, self.current_player) == 1000:
            messagebox.showinfo("Game Over", f"Player {self.current_player} wins!")
            self.root.destroy()
        elif compute_utility(self.game, self.current_player) == -1000:
            messagebox.showinfo("Game Over", f"Player {next_player(self.current_player)} wins!")
            self.root.destroy()
